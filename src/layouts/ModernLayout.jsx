...
export default function ModernLayout(props) {
  const {
    config, t,
    utcTime, utcDate, localTime, localDate,
    localWeather, dxWeather, spaceWeather, solarIndices,
    use12Hour, handleTimeFormatToggle,
    setShowSettings, handleUpdateClick, handleFullscreenToggle,
    isFullscreen, updateInProgress, isLocalInstall,
    leftSidebarVisible, rightSidebarVisible,
    scale,
    deGrid, dxGrid,
    dxLocation, dxLocked, handleDXChange, handleToggleDxLock,
    deSunTimes, dxSunTimes,
    tempUnit, setTempUnit, showDxWeather,
  ...
  const deLocationPanel = (
    <div className="panel" style={{ padding: isMobile ? '10px' : '14px', flex: '0 0 auto' }}>
      <div style={{ fontSize: '14px', color: 'var(--accent-cyan)', fontWeight: '700', marginBottom: '10px' }}>{t('app.dxLocation.deTitle')}</div>
  ...
  const dxLocationPanel = (
    <div className="panel" style={{ padding: isMobile ? '10px' : '14px', flex: '0 0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <div style={{ fontSize: '14px', color: 'var(--accent-green)', fontWeight: '700' }}>{t('app.dxLocation.dxTitle')}</div>
        <button onClick={handleToggleDxLock} title={dxLocked ? t('app.dxLock.unlockTooltip') : t('app.dxLock.lockTooltip')} style={{ background: dxLocked ? 'var(--accent-amber)' : 'var(--bg-tertiary)', color: dxLocked ? '#000' : 'var(--text-secondary)', border: '1px solid ' + (dxLocked ? 'var(--accent-amber)' : 'var(--border-color)'), borderRadius: '4px', padding: '2px 6px', fontSize: '10px', cursor: 'pointer' }}>
          {dxLocked ? '🔒' : '🔓'}
        </button>
      </div>
      <div style={{ fontFamily: 'JetBrains Mono', fontSize: '14px' }}>
        <div style={{ color: 'var(--accent-green)', fontSize: '22px', fontWeight: '700', letterSpacing: '1px' }}>{dxGrid}</div>
        ...
        <div style={{ marginTop: '8px', display: 'flex', gap: '16px', fontSize: '13px' }}>
          <span>
            ...
            <span style={{ color: 'var(--accent-amber)', fontWeight: '600' }}>
              {t('app.dxLocation.sp')} {(() => { const dLon = ((dxLocation.lon - config.location.lon) * Math.PI) / 180; const dLat1 = (config.location.lat * Math.PI) / 180; const dLat2 = (dxLocation.lat * Math.PI) / 180; const y = Math.sin(dLon) * Math.cos(dLat2); const x = Math.cos(dLat1) * Math.sin(dLat2) - Math.sin(dLat1) * Math.cos(dLat2) * Math.cos(dLon); return `${Math.round(((Math.atan2(y, x) * 180) / Math.PI + 360) % 360)}°`; })()}
            </span>
          </span>
          <span>
            <span style={{ color: 'var(--text-muted)' }}>{t('app.dxLocation.sp')} </span>
            <span style={{ color: 'var(--accent-cyan)', fontWeight: '600' }}>
              {(() => { const R = 6371; const dLat1 = (config.location.lat * Math.PI) / 180; const dLon1 = (config.location.lon * Math.PI) / 180; const dLat2 = (dxLocation.lat * Math.PI) / 180; const dLon2 = (dxLocation.lon * Math.PI) / 180; const dLat = dLat2 - dLat1; const dLon = dLon2 - dLon1; const a = Math.sin(dLat / 2) ** 2 + Math.cos(dLat1) * Math.cos(dLat2) * Math.sin(dLon / 2) ** 2; return `📏 ${Math.round(R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))).toLocaleString()} km`; })()}
            </span>
          </span>
        </div>
        <div style={{ marginTop: '8px', fontSize: '13px' }}>
  ...
  const dxCluster = config.panels?.dxCluster?.visible !== false && (
    <DXClusterPanel
      data={dxClusterData.spots} loading={dxClusterData.loading} totalSpots={dxClusterData.totalSpots}
      filters={dxFilters} onFilterChange={setDxFilters} onOpenFilters={() => setShowDXFilters(true)}
      onHoverSpot={setHoveredSpot} onSpotClick={handleDXSpotClick} hoveredSpot={hoveredSpot}
      showOnMap={mapLayers.showDXPaths} onToggleMap={toggleDXPaths}
    />
  );

  const pskPanel = config.panels?.pskReporter?.visible !== false && (
    <PSKReporterPanel
      callsign={config.callsign} pskReporter={pskReporter}
      showOnMap={mapLayers.showPSKReporter} onToggleMap={togglePSKReporter}
      filters={pskFilters} onOpenFilters={() => setShowPSKFilters(true)}
      onShowOnMap={(r) => { if (r.lat && r.lon) handleDXChange({ lat: r.lat, lon: r.lon }); }}
      wsjtxDecodes={wsjtx.decodes} wsjtxClients={wsjtx.clients} wsjtxQsos={wsjtx.qsos}
      wsjtxStats={wsjtx.stats} wsjtxLoading={wsjtx.loading} wsjtxEnabled={wsjtx.enabled}
      wsjtxPort={wsjtx.port} wsjtxRelayEnabled={wsjtx.relayEnabled}
      wsjtxRelayConnected={wsjtx.relayConnected} wsjtxSessionId={wsjtx.sessionId}
      showWSJTXOnMap={mapLayers.showWSJTX} onToggleWSJTXMap={toggleWSJTX}
    />
  );

  const potaSotaPanel = config.panels?.pota?.visible !== false && (
    <PotaSotaPanel
      potaData={potaSpots.data} potaLoading={potaSpots.loading} potaLastUpdated={potaSpots.lastUpdated} potaLastChecked={potaSpots.lastChecked}
      showPOTA={mapLayers.showPOTA} onTogglePOTA={togglePOTA}
      sotaData={sotaSpots.data} sotaLoading={sotaSpots.loading} sotaLastUpdated={sotaSpots.lastUpdated} sotaLastChecked={sotaSpots.lastChecked}
  ...
  const headerEl = (
    <Header
      config={config} utcTime={utcTime} utcDate={utcDate} localTime={localTime} localDate={localDate}
      localWeather={localWeather} spaceWeather={spaceWeather} solarIndices={solarIndices}
      bandConditions={bandConditions} use12Hour={use12Hour} onTimeFormatToggle={handleTimeFormatToggle}
      onSettingsClick={() => setShowSettings(true)} onUpdateClick={handleUpdateClick}
      onFullscreenToggle={handleFullscreenToggle} isFullscreen={isFullscreen}
      updateInProgress={updateInProgress} showUpdateButton={isLocalInstall}
  ...
  if (isMobile) {
    ...
    const mobileCard = (child, key, opts = {}) => (
      <div key={key} style={{ width: '100%', minHeight: opts.minH || 'auto', flexShrink: 0 }}>{child}</div>
    );
    ...
    return (
      <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          WebkitOverflowScrolling: 'touch', scrollSnapType: 'y proximity',
          padding: '4px', display: 'flex', flexDirection: 'column', gap: '6px',
        }}>
          ...
          {/* Propagation */}
          {config.panels?.propagation?.visible !== false && mobileCard(
            <PropagationPanel propagation={propagation.data} loading={propagation.loading} bandConditions={bandConditions} units={config.units} propConfig={config.propagation} />,
            'prop'
          )}

          {/* DXpeditions */}
          {config.panels?.dxpeditions?.visible !== false && mobileCard(
            <DXpeditionPanel data={dxpeditions.data} loading={dxpeditions.loading} />, 'dxped'
          )}
          ...
          {/* Contests */}
          {config.panels?.contests?.visible !== false && mobileCard(
            <ContestPanel data={contests.data} loading={contests.loading} />, 'contests'
          )}
  ...
  if (isTablet) {
    return (
      <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '4px 6px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              {config.panels?.dxpeditions?.visible !== false && <DXpeditionPanel data={dxpeditions.data} loading={dxpeditions.loading} />}
              {potaSotaPanel}
              {config.panels?.contests?.visible !== false && <ContestPanel data={contests.data} loading={contests.loading} />}
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      {rightSidebarVisible && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', overflow: 'hidden' }}>
          ...
          {pskPanel && (
            <div style={{ flex: `${config.panels?.pskReporter?.size || 2} 1 0`, minHeight: '120px', overflow: 'hidden' }}>
              {pskPanel}
            </div>
          )}
          {config.panels?.dxpeditions?.visible !== false && (
            <div style={{ flex: `${config.panels?.dxpeditions?.size || 1} 1 0`, minHeight: '60px', overflow: 'hidden' }}>
