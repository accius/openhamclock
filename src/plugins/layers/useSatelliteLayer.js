...
export const useLayer = ({ map, enabled, satellites, setSatellites, opacity, config, units }) => {
  ...
  // Function to clear all selections added
  const clearAllSats = () => {
    setSelectedSats([]);
  };
  ...
  useEffect(() => {
    ...
    window.clearAllSats = () => clearAllSats(); // Exposed to window for HTML string access
  }, [selectedSats]);
  ...
  const updateInfoWindow = () => {
    ...
    if (!winMinimized) {
      ...
      // Inserted Clear All Footprints Button
      html += `
        <div style="padding-top: 5px;">
          <button class="sat-clear-btn" onclick="window.clearAllSats()">Clear All Footprints</button>
        </div>
      `;
