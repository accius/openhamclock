...
const resolveConfigPath = () => {
  return (
    process.env.DXSPIDER_PROXY_CONFIG || path.join(__dirname, "config.json")
  );
};
...
const loadFileConfig = () => {
  ...
  if (!fs.existsSync(configPath)) {
    return { configPath, fileConfig: null };
  }

  try {
    const raw = fs.readFileSync(configPath, "utf8");
    const parsed = JSON.parse(raw);
    return { configPath, fileConfig: parsed };
  } catch (err) {
...
const { configPath, fileConfig } = loadFileConfig();
...
const CONFIG = {
  ...
  callsign:
    ...
    (fileConfig && fileConfig.callsign) ||
    DEFAULT_CONFIG.callsign,
  port:
    process.env.PORT || (fileConfig && fileConfig.port) || DEFAULT_CONFIG.port,
  logLevel:
    process.env.LOG_LEVEL ||
    (fileConfig && fileConfig.logLevel) ||
    DEFAULT_CONFIG.logLevel,
  nodes:
    fileConfig && Array.isArray(fileConfig.nodes) && fileConfig.nodes.length > 0
      ? fileConfig.nodes
...
const LOG_LEVEL = String(CONFIG.logLevel || "info").toLowerCase();
...
const log = (level, message, data = null) => {
  const categoryLevel =
    LOG_LEVELS[CATEGORY_LEVELS[level] || "info"] ?? LOG_LEVELS.info;
...
const parseSpotLine = (line) => {
  try {
    ...
    if (timeMatch) {
      time =
        timeMatch[1].substring(0, 2) + ":" + timeMatch[1].substring(2, 4) + "z";
    ...
    if (upperComment.includes("FT8")) mode = "FT8";
      ...
      upperComment.includes("SSB") ||
      upperComment.includes("USB") ||
      upperComment.includes("LSB")
    )
      mode = "SSB";
    ...
    if (dualGridMatch) {
      ...
      const singleGridMatch = comment.match(
        /\b([A-R]{2}[0-9]{2}(?:[A-X]{2})?)\b/i,
      );
      if (singleGridMatch) {
...
const cleanupSpots = () => {
  ...
  if (removed > 0) {
    log(
      "CLEANUP",
      `Removed ${removed} expired spots, ${spots.length} remaining`,
    );
  }
};
...
const connect = () => {
  ...
  log(
    "CONNECT",
    `Attempting connection to ${node.name} (${node.host}:${node.port})`,
  );
...
const handleDisconnect = () => {
  ...
  if (reconnectAttempts >= CONFIG.maxReconnectAttempts) {
    ...
    log(
      "FAILOVER",
      `Switching to node: ${CONFIG.nodes[currentNodeIndex].name}`,
    );
  }

  log(
    "RECONNECT",
    `Attempting reconnect in ${CONFIG.reconnectDelayMs}ms (attempt ${reconnectAttempts})`,
  );
...
app.get("/api/debug/grids", (req, res) => {
  const spotsWithGrids = spots
    .filter((s) => s.dxGrid || s.spotterGrid)
    .slice(0, 20);
  ...
  if (typeof index === "number" && index >= 0 && index < CONFIG.nodes.length) {
    ...
    res
      ...
      .json({
        ...
        availableNodes: CONFIG.nodes.map((n) => n.name),
      });
  }
});
...
app.listen(PORT, () => {
  ...
  log(
    "CONFIG",
    `Available nodes: ${CONFIG.nodes.map((n) => n.name).join(", ")}`,
  );
  if (fileConfig) {
    log("CONFIG", `Loaded config file: ${configPath}`);
  } else {
    log("CONFIG", `No config file found at ${configPath}, using defaults/env`);
  }
